...
export function useRouterAddress(trade?: TradeInfo): Address {
  return useMemo(() => {
    ...
    const chainId = ChainId.XLAYER_TESTNET // TODO: 从context获取当前chainId
    const v2Router = getContractAddress(chainId, ContractType.V2_ROUTER)
    const smartRouter = getContractAddress(chainId, ContractType.SMART_ROUTER)
    ...
    if (useV2) {
      ...
      return v2Router as Address
    }

    // 🛡️ 安全检查：Smart Router是否已部署
    if (!smartRouter || smartRouter === '0x0000000000000000000000000000000000000000') {
...
export function getRouterAddress(chainId: ChainId, routerType: 'V2' | 'SMART'): Address {
  switch (routerType) {
    case 'V2':
      return getContractAddress(chainId, ContractType.V2_ROUTER) as Address
    case 'SMART':
      return getContractAddress(chainId, ContractType.SMART_ROUTER) as Address
    default:
