...
const StyledButton = styled("button").withConfig({
  ...
  align-items: center;
  border: 0;
  border-radius: ${({ theme }: { theme: any }) => theme.radii.default}; /* CometSwap: 使用更新的圆角值 8px */
  box-shadow: ${({ theme }: { theme: any }) => theme.shadows.button}; /* CometSwap: 使用现代化阴影 */
  cursor: pointer;
  display: inline-flex;
  font-family: inherit;
  font-size: 16px;
  ...
  @media (hover: hover) {
    &:hover:not(:disabled):not(.comet-button--disabled):not(.comet-button--disabled):not(:active) {
      ...
      box-shadow: ${({ theme }: { theme: any }) => theme.shadows.buttonHover};
  ...
  &:active:not(:disabled):not(.comet-button--disabled):not(.comet-button--disabled) {
    ...
    box-shadow: ${({ theme }: { theme: any }) => theme.shadows.button};
  }
