...
const StyledCardRibbon = styled.div<Partial<StyledCardRibbonProps>>`
  z-index: ${({ theme }: { theme: any }) => theme.zIndices.ribbon};
  background-color: ${({ variantColor = "secondary", theme }: { variantColor?: string; theme: any }) => theme.colors[variantColor]};
  color: white;
  margin: 0;
  padding: 8px 0;
  position: absolute;
  right: ${({ ribbonPosition  }: { ribbonPosition: any }) => (ribbonPosition === "right" ? 0 : "auto")};
  top: 0;
  text-align: center;
  transform: ${({ ribbonPosition  }: { ribbonPosition: any }) =>
