...
const StyledInputGroup = styled(Box)<{ scale: Scales; hasStartIcon: boolean; hasEndIcon: boolean }>`
  ${Input} {
    padding-left: ${({ hasStartIcon, scale  }: { hasStartIcon: any, scale: any }) => getPadding(scale, hasStartIcon)};
    padding-right: ${({ hasEndIcon, scale  }: { hasEndIcon: any, scale: any }) => getPadding(scale, hasEndIcon)};
  }
`;
...
const InputIcon = styled.div<{ scale: Scales; $isEndIcon?: boolean }>`
  ...
  ${({ $isEndIcon, scale  }: { $isEndIcon: any, scale: any }) =>
    $isEndIcon
      ? `
