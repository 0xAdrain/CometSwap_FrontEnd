import { useTheme } from "@comet-swap/hooks";
...
const SelectContainer = styled.div`
  ...
  .p-multiselect-panel {
    ...
    box-shadow: ${({ theme }: { theme: any }) => theme.card.boxShadow};
  ...
  .p-checkbox {
    ...
    .p-checkbox-box {
      ...
      border: 1px solid ${({ theme }: { theme: any }) => theme.colors.inputSecondary};
      width: ${CHECKBOX_WIDTH};
      height: ${CHECKBOX_WIDTH};
      border-radius: 8px;
      background-color: ${({ theme }: { theme: any }) => theme.colors.input};
    }

    &.p-highlight {
      .p-checkbox-box {
        border-color: ${({ theme }: { theme: any }) => theme.colors.success};
        background-color: ${({ theme }: { theme: any }) => theme.colors.success};
      }
      ...
      .p-checkbox-icon.p-icon {
        ...
        color: ${({ theme }: { theme: any }) => theme.colors.backgroundAlt};
        
        path {
          stroke: ${({ theme }: { theme: any }) => theme.colors.backgroundAlt};
          stroke-width: 1;
        }
      }
    }
...
const PrimereactSelectContainer = styled.div<{ $scrollHeight?: string }>`
  ...
  .p-multiselect-items-wrapper {
    height: ${({ $scrollHeight  }: { $scrollHeight: any }) => $scrollHeight ?? "auto"};
  }
  .p-multiselect-items {
    height: 100%;
  }
...
const SelectInputContainer = styled(Flex)`
  ...
  border-radius: ${BORDER_RADIUS};
  box-shadow: 0 0 1px ${({ theme }: { theme: any }) => theme.shadows.inset};
...
const SelectedInputIconsContainer = styled.div`
  ...
  & > img {
    width: 28px;
    height: 28px;
    border: 2px solid ${({ theme }: { theme: any }) => theme.colors.input};
    border-radius: 50%;
  }
...
const SelectedInputFakeIcon = styled.span`
  ...
  width: 24px;
  height: 24px;
  background: ${({ theme }: { theme: any }) => theme.colors.inputSecondary};
...
const SelectInputPlaceholder = styled.div`
  width: 100%;
  color: ${({ theme }: { theme: any }) => theme.colors.textSubtle};
`;
