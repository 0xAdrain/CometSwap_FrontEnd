...
const StyledBox = styled(Box)<{ isFocus: boolean }>`
  ...
  padding: 5px 8px;
  min-height: 42px;
  ${({ isFocus  }: { isFocus: any }) => isFocus && `box-shadow: 0px 0px 0px 4px #7645D933, 0px 0px 0px 1px #7645D9;`}
`;
...
const SelectedLabel = styled.span`
  ...
  border-radius: ${BORDER_RADIUS};
  border: 1px solid ${({ theme }: { theme: any }) => theme.colors.inputSecondary};
  background-color: ${({ theme }: { theme: any }) => theme.colors.textSubtle};
  color: ${({ theme }: { theme: any }) => theme.card.background};
  gap: 4px;
`;
