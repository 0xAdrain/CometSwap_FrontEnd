...
const ConcaveContainer = styled(Box)<ContainerProps>`
  ${({ theme, clipPath, clipFill  }: { theme: any, clipPath: any, clipFill: any }) => sharedStyles(theme, clipPath, clipFill)}
  transform: ${({ clipPath  }: { clipPath: any }) => (clipPath === "#bottomConcaveCurve" ? "translate(0, -13px)" : "translate(0, 1px)")};
`;

const ConvexContainer = styled(Box)<ContainerProps>`
  ${({ theme, clipPath, clipFill  }: { theme: any, clipPath: any, clipFill: any }) => sharedStyles(theme, clipPath, clipFill)}
  transform: ${({ clipPath  }: { clipPath: any }) => (clipPath === "#bottomConvexCurve" ? "translate(0, -13px)" : "translate(0, -1px)")};
`;
