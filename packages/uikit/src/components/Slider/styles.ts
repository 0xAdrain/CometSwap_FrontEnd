...
export const SliderLabel = styled(Text)<SliderLabelProps>`
  bottom: 0;
  font-size: 12px;
  left: ${({ progress  }: { progress: any }) => progress};
  position: absolute;
  text-align: center;
...
export const BunnyButt = styled.div<DisabledProp>`
  ...
  height: 32px;
  filter: ${({ disabled  }: { disabled: any }) => (disabled ? "grayscale(100%)" : "none")};
  position: absolute;
  width: 15px;
`;
...
export const BarBackground = styled.div<DisabledProp>`
  background-color: ${({ theme, disabled  }: { theme: any, disabled: any }) => theme.colors[disabled ? "textDisabled" : "inputSecondary"]};
  height: 2px;
  position: absolute;
  top: 18px;
  width: 100%;
