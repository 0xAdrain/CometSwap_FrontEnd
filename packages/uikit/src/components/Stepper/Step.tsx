...
const StyledStep = styled(Flex)`
  ${({ theme }: { theme: any }) => theme.mediaQueries.md} {
...
const Connector = styled.div<StatusProps>`
  ...
  width: 4px;
  height: calc(50% + 20px);
  ${({ $isFirstStep, $isLastStep, $isFirstPart  }: { $isFirstStep: any, $isLastStep: any, $isFirstPart: any }) => {
...
const ChildrenWrapper = styled(Box)<{ isVisible: boolean }>`
  ${({ theme }: { theme: any }) => theme.mediaQueries.md} {
    visibility: ${({ isVisible  }: { isVisible: any }) => (isVisible ? "visible" : "hidden")};
  }
`;
...
export const StepNumber = styled.div<StatusProps>`
  ...
  background-color: ${({ theme, status  }: { theme: any, status: any }) => theme.colors[status === "current" ? "secondary" : "invertedContrast"]};
  border: 2px solid ${({ theme, status  }: { theme: any, status: any }) => (status === "past" ? theme.colors.success : "transparent")};
  border-radius: ${({ theme }: { theme: any }) => theme.radii.circle};
