...
const SortButton = styled(Button)<ISortArrowButton>`
  ...
  width: ${({ width  }: { width: any }) => width ?? "25px"};
  height: ${({ height, width  }: { height: any, width: any }) => height ?? width ?? "25px"};
  border-color: ${({ theme }: { theme: any }) => theme.colors.cardBorder};
  background: ${({ theme }: { theme: any }) => (theme.isDark ? theme.colors.backgroundDisabled : theme.colors.input)};
  path {
    fill: ${({ theme }: { theme: any }) => (theme.isDark ? "rgba(255, 255, 255, 0.2)" : "#B4ACCF")};
  }
  &.descend {
    background: ${({ theme }: { theme: any }) => (theme.isDark ? theme.colors.input : theme.colors.textSubtle)};
    path:first-child {
...
const SortDESCButton = styled(Button)<ISortArrowButton>`
  ...
  background: ${({ theme }: { theme: any }) => (theme.isDark ? theme.colors.backgroundDisabled : theme.colors.input)};
  path {
    fill: ${({ theme }: { theme: any }) => (theme.isDark ? "rgba(255, 255, 255, 0.2)" : theme.colors.textSubtle)};
  }
  &.descend {
    background: ${({ theme }: { theme: any }) => (theme.isDark ? theme.colors.input : theme.colors.textSubtle)};
    path:first-child {
